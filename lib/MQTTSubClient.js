...
var MQTTSubClient = AbstractComponent.extend({
    ...
    start: function (done) {
        var host  = this.dictionary.getString('host'),
            ...
            topic = this.dictionary.getString('topic', '/');
        ...
        if (host && host.length > 0 && port !== null) {
            ...
            this.client.on('message', function (topic, msg) {
                ...
                try {
                    ...
                    this.log.error(this.toString(), 'Unable to serialize topic and message to JSON, onTopicAndMsg port will not be used');
                }
            }.bind(this));
